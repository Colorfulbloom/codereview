...
pub fn run_init() -> Result<()> {
    ...
    // 2. Languages
    println!("\n{}", bold.apply_to("Step 2: Languages"));
    select_languages(&mut yaml)?;

    // 3. Rule overrides
    println!("\n{}", bold.apply_to("Step 3: Rule Overrides"));
    configure_rules(&mut yaml)?;

    // 4. Custom rules
    println!("\n{}", bold.apply_to("Step 4: Custom Rules"));
    add_custom_rules(&mut yaml)?;

    // 5. Custom agents
    println!("\n{}", bold.apply_to("Step 5: Custom Agents"));
    println!(
        "{}",
        dim.apply_to(
